...
def run_pipeline(
    ...
    if from_run:
        ...
        run_dir = root / "output" / f"{slugify(topic)}-{timestamp}"
    ...
    if cluster_mode == "llm":
        ...
        context_call = research_game_context(
            settings.toapis_api_key, topic, candidates, model=context_model
        )
        ...
        write_json(run_dir / "llm" / "game-context.json", context_call.data)
