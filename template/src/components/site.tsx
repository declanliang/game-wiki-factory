...
export async function SiteHeader({ locale }: { locale: string }) {
  ...
  const navGroupsByKey = Object.fromEntries(getDynamicNavigation(locale as Locale).map((group) => [group.slug, group]));
  const navItems = NAVIGATION_CONFIG.map((item) => {
    ...
    const articles = group?.links.slice(1).map((link) => ({ label: link.label, href: link.href }));
    return { key: item.key, path: item.path, label: t(item.key), articles, readMoreLabel: shared("readMore") };
  });
  const header = (
    <div className="flex items-center justify-between gap-4">
      <Link href={localizeHref("/", locale)} className="flex items-center gap-3">
        <span className="grid h-9 w-9 place-items-center rounded-xl border border-border bg-muted text-sm font-black text-foreground">{shortName.slice(0, 2).toUpperCase()}</span>
          ...
          {socialLinks.length > 0 && (
            <div className="mb-4 flex flex-wrap justify-center gap-3">
              {socialLinks.map(([label, href]) => (
                <Link key={href} href={href} className="inline-flex items-center gap-1.5 rounded-full border border-border px-4 py-1.5 text-sm font-medium text-muted-foreground transition hover:border-[hsl(var(--nav-theme-light))] hover:text-foreground">
