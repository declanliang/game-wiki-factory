...
游戏目录的 `package.json` 位于仓库根，因此 Vercel Root Directory 留空。部署只运行确定性 Next.js build；所有 AI 工作在本地工厂完成。正式部署必须设置 `NEXT_PUBLIC_SITE_URL`。模板集中把裸域名规范为 HTTPS origin，并让 metadata、JSON-LD、sitemap 与 robots 共享 locale-aware URL 构造器；非英语页面必须 self-canonical。部署验收拒绝重定向 sitemap URL、域名后双斜杠、canonical/hreflang 冲突和 metadata 缺失。
