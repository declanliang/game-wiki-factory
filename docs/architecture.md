...
游戏名 + 可选 platform / official URL
  → pipeline/basic-info
      ...
      官方身份、规范事实、首页、hero/favicon、默认 en/es、category candidates
...
3. `site-plan.json` 把 Guide Search 主题限制在 profile 内，记录分类顺序、已声明语言标签与描述、主关键词、研究查询、用户问题、必答点、独特价值、页面类型、实体/意图元数据、交付数量及状态。`strategy/tips/tactics` 合并到 `guide` 分类但保留独立关键词；planner 不生成无证据的 fallback 主题。有限共享背景属于允许的内容复用，错误游戏/无关内容才进入删除门。
...
- Basic Info：已声明语言首页、hero、favicon 和验证报告完整才可复用；cache 位于 `.gamewiki/basic-info/.cache/`。
