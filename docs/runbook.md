...
- 英文页面首次生成和修复后仍因高推测密度或未经证明的 Steam Deck 正面等级不合格：SEO Scout 将其作为证据不足页面淘汰，写入 `out/generation_rejections.json`，并继续处理有证据的页面。同一证据指纹不会重复付费生成；格式、截断、API 等其他错误仍会停止任务。不要为凑页面放宽事实门或手工伪造文章。
