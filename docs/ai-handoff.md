...
- 有正式域名时填写 `siteUrl`。后台任务自动发布 Private GitHub、创建连接该 repo `main` 的 Git-integrated Pages 项目、设置 `NEXT_PUBLIC_SITE_URL`、创建或复用同名 Custom Domain 并触发 Cloudflare 构建部署；仅在 Cloudflare 返回 DNS/验证 pending 时，由运营者按控制台指示完成该动作，禁止 `example.com` 出现在真实生产 sitemap/canonical。
