...
- 后台站点任务默认发布 Private GitHub，并以 `hosting.provider=cloudflare-pages`、`hosting.status=manual_action_required` 提醒运营者连接仓库、把构建输出设为 `out`、绑定域名、设置 `NEXT_PUBLIC_SITE_URL`、部署和执行线上验收。Job 的 `succeeded` 不得表述为“已上线”。
