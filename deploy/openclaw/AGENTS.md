...
- `gamewiki-supervisor.timer` owns deterministic recovery of checkpoint-safe article/translation failures. `gamewiki-agent2.timer` owns bounded Codex CLI recovery of single-project content, MDX, metadata, slug/checkpoint projection, and transient publish issues. Do not duplicate their work, create a replacement job, or repeatedly ask the user to retry. Report only the final notification after their bounded recovery budgets are exhausted.
...
- v1_0728 new sites generate only English and Spanish. English is public immediately; one internal `localeRelease` job publishes Spanish on the third following natural day through a Git commit and Cloudflare Pages. `de/fr/ja` are not Factory work and belong to the approved Growth Agent locale-expansion workflow. Do not cancel, recreate, or manually accelerate the Spanish release unless the user explicitly asks. A locale-release failure does not invalidate the live English site.
