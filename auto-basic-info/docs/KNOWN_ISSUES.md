...
进展：语言市场已从通用 Task 2 拆成独立联网任务。当前按 SEO 广告收入策略固定调查 `en/es/de/fr/ja/ko/it/nl`，`en/es` 兜底，额外语言用开发者本地化或两个独立来源作为入选门槛；中文及低优先级广告市场不消耗调研预算。仍可继续把官方链接与代码/玩法拆开，或使用能在请求前确定费用的搜索层。
...
当前 `TEMPLATE_SITE_IDENTITY_SCHEMA` 与 `TEMPLATE_SITE_CONTENT_SCHEMA` 根据 `game-wiki-template/doc/homepage-info-schema.md`、`check-intake.mjs` 和 `apply-content.mjs` 实现；最终包还会强校验一张 Hero 与 favicon 7 文件。两个仓库独立修改时，模板契约仍可能变化而本项目未同步。
