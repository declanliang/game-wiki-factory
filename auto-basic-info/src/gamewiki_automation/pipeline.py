...
from .prompts import SYSTEM_JSON, homepage_prompt, language_market_prompt, localized_site_content_prompt, localized_site_content_revision_prompt, localized_value_revision_prompt, modules_prompt, research_prompt
from .report import render_basic_info
from .roblox import RobloxClient
from .schemas import DEFAULT_LANGUAGE_CODES, HOMEPAGE_SCHEMA, LANGUAGE_MARKET_SCHEMA, MODULES_SCHEMA, MONETIZATION_LANGUAGE_CODES, RESEARCH_SCHEMA, TEMPLATE_SITE_CONTENT_SCHEMA
...
class Pipeline:
    ...
    def run(self, game_name: str) -> tuple[Path, dict[str, Any]]:
        ...
        language_market, language_meta = self.llm.generate(
            "language_market_research",
            SYSTEM_JSON,
            language_market_prompt(_language_research_facts(facts)),
            LANGUAGE_MARKET_SCHEMA,
            web=True,
            ttl=14 * 86400,
        )
        dump_json(raw_dir / "language-market-research.json", {"data": language_market, "meta": language_meta})
        facts, evidence = self._merge_language_market(facts, evidence, language_market)
        ...
        dump_json(raw_dir / "homepage-generation.json", {"data": homepage, "meta": homepage_meta, "generatedLanguages": generated_languages, "languageNormalization": "Replaced with dedicated language-market selection."})
...
def _homepage_languages(facts: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for rank, code in enumerate(facts.get("languages") or DEFAULT_LANGUAGE_CODES, 1):
        ...
        result.append({
            "rank": rank,
            "code": code,
            "language": candidate.get("language") or ("English" if code == "en" else code),
            "localizedSiteName": f"{name} Wiki",
            "gameName": name,
            "basis": "evidence" if source_urls else "inference",
            "sourceUrls": source_urls,
            "confidence": float(candidate.get("confidence") or (0.6 if code == "en" else 0.5)),
        })
    return result
