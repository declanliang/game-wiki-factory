...
- `game-wiki-template` 消费同级的 `site-identity.json`、`site-content.json` 与每个非英语语言的 `site-content.<locale>.json`；不得把内部 `00首页信息.json` 直接复制给模板。
...
- `site-identity.json` 只能有规范的 7 个大写 key（含 `LANGUAGES`）；`en/es` 固定兜底，只额外调研 `de/fr/ja/ko/it/nl`，永不考虑中文，其他语言不进入候选。非默认语言必须通过独立语言市场调研门槛。`site-content.json` 顶层只能有 `site/home`。
