...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument(
        "--run-dir",
        type=Path,
        help="Write a new collection to this exact directory (used by the project orchestrator).",
    )
    result.add_argument(
        "--refresh-source",
        action="append",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.trusted_context_file:
        trusted_context = json.loads(
            args.trusted_context_file.resolve().read_text(encoding="utf-8")
        )
        if not isinstance(trusted_context, dict):
            parser().error("--trusted-context-file must contain a JSON object")
    if args.from_run:
        settings = None
