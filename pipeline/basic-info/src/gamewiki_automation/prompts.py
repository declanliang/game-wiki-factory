...
2. English is the only business-policy default and must be include. Spanish and other languages are post-launch Growth decisions, not default site generation.
...
Decision rules:
- English is always included as the monetization/coverage fallback. Other candidates must pass the evidence threshold, but they are not generated by the core Factory unless a post-launch Growth workflow explicitly requests them.
