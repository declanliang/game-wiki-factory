...
- `site-identity.json` 只能有规范的 7 个大写 key（含 `LANGUAGES`）；Factory 新站语言策略必须严格为 `en`，不在建站阶段执行单游戏语言市场调研。`es/de/fr/ja` 属于上线后的 Growth Agent 扩展。`site-content.json` 顶层只能有 `site/home`。
