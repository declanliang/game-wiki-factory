...
cd game-wiki-factory/pipeline/seo-scout
pip install -e .
```
...
由总编排器调用时，使用精确项目目录，确保真实游戏不写入源码仓库：

```bash
python -m seoscout --project-dir /path/to/my-game/.gamewiki/content-pipeline run --keywords /path/to/my-game/.gamewiki/planning/seo-keywords.json
```

`--project-dir` 必须放在子命令 `run` 之前。独立使用且不传该参数时，仍使用 `config.json.output_dir/<project>`。
...
| 文件 | 放什么 | 会不会进 git |
|------|--------|------|
| `../../.env` | factory 统一维护的密码/key/凭证 | ❌ 被 `.gitignore` 排除，每个人/每台机器自己填一份 |
...
密钥统一填写在 factory 根目录 `.env`，不要在本模块维护第二份：
...
若某个 key 明确返回余额不足，当前进程会禁用该 key；所有 key 都无额度时会停止后续批次，保留已经验证落盘的文章和翻译。充值后直接重跑原命令，不要加 `--overwrite`。
...
| `translate.reasoning_effort` | `none` | 翻译是结构保留任务；Gemini 2.5 Flash 关闭 thinking，避免隐藏推理 token 挤占正文。 |
...
直接重跑同一命令，不要加 `--overwrite`。`translate` 会把每个现有翻译与英文源文对照，检查标题层级、列表、表格、FAQ、Callout、长度和结尾；不完整 checkpoint 会被删除并单独重试。API 返回的 `finish_reason` 不是 `stop` 时，客户端不会保存响应。Gemini 2.5 Flash 建议保留 `translate.reasoning_effort: "none"`。
