...
- Guide Search 调用 Google Suggest 主词和 a–z，并可排序/淘汰关键词，但不能越过 profile 创建无关分类。
- `site-plan.json` 是分类、顺序、六语言标签和发布状态的唯一事实源。
- 每站至少 4、最多 8 个 published 分类。
...
5. 完成时必须确认：至少 4 个分类、六语言文章树一致、intake 通过、TypeScript 通过、production build 通过、sitemap 全 URL 为 200、hreflang 完整。
